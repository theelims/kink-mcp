...
|---|---|---|
| `kink_mcp/config.py` | **Create** | Platform-aware config path; load/save JSON |
| `kink_mcp/ui.py` | **Create** | aiohttp app; all API routes; single-page HTML/CSS/JS |
| `kink_mcp/device.py` | **Modify** | Add `_device_meta`, `add_offline_device()`, `get_device_list()`, `disconnect_one()`, `rename_alias()`; rename `set_strength_limit` → `set_pain_limit` |
| `kink_mcp/server.py` | **Modify** | Remove scan/connect/disconnect tools; conditional `set_pain_limit`; add `ui://url` resource; rewrite `main()` |
...
Expected: `ModuleNotFoundError: No module named 'kink_mcp.config'`

- [ ] **Step 4: Implement `kink_mcp/config.py`**

```python
"""Persistent JSON config for kink-mcp."""
...
```bash
git add kink_mcp/config.py tests/__init__.py tests/test_config.py
...
In `kink_mcp/device.py`, find `class DeviceManager:` and update `__init__`:
...
## Task 4: UI server (`kink_mcp/ui.py`)

**Files:**
- Create: `kink_mcp/ui.py`

- [ ] **Step 1: Create `kink_mcp/ui.py`**
...
Replace the entire `kink_mcp/server.py` with:
...
```bash
uv run python -c "import kink_mcp.server"
```
